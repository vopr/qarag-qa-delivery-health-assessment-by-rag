...
You are the Preferences/Config assistant/skill for the QARAG metric assessment system.
...
Replace section **## 5) STORAGE-XHTML WRAPPER + PREFLIGHT VALIDATION (MUST)** with this adjusted version (same content, but with your preferred ending):
...
2) **Prefight validation** (fail fast):
...
3) If Confluence still rejects the update, try to understand the root cause and fix it (3 times), only after this treat it as a hard value:
